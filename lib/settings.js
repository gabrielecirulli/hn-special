function Settings() {
  ...
  // Load the settings
  _.request(chrome.extension.getURL("lib/defaults.json"), "GET", function (code) {
...
Settings.prototype.buildMenuFrame = function (container, showTip) {
  ...
  button.appendChild(_.createElement("img", {
    ...
    attributes: {
      src: chrome.extension.getURL("resources/gear.svg")
    }
  }));
...
Settings.prototype.setupOptionalPermissions = function (container, moduleName) {
  // Remove the iframe if it exists
  var element = document.getElementById('hnspecial-permissions');
  if (element) {
  ...
  var iframe = _.createElement("iframe", {
    attributes: {
      id: 'hnspecial-permissions',
      src: chrome.extension.getURL('lib/extras/permissions.html#' + moduleName),
      "class": "hnspecial-permissions-iframe"
    }
  });
  ...
  Object.keys(permissions).forEach(function (key) {
    if (map[key]) {
      ...
      checkbox.addEventListener("change", function (e) {
        ...
        var permissionsContainer = e.target.parentNode.parentNode;
        self.setupOptionalPermissions(permissionsContainer, key);
      });
    }
  });
};
