...
function BrowserAbstraction()
{
  ...
  this.getUrl = function( name )
  {
    return HNSpecial.isChrome
      ? HNSpecial.chrome.runtime.getURL( name )
      : this.firefoxOptions.urlBase + name;
  };

  this.getDefaultOptions = function( cb )
  {
    return HNSpecial.isChrome
      ? _request( HNSpecial.browser.getUrl( "lib/defaults.json" ), "GET", cb )
...
function Settings() {
  ...
  console.error( HNSpecial.browser );
